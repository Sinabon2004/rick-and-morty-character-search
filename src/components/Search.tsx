...
export default function Search({ onCharactersUpdate }: SearchProps) {
  ...
  const debouncedQuery = useCallback(async () => {
    if (query.length > 3) {
      try {
        const characters = await fetchCharacters(query);
        onCharactersUpdate(characters.results);
        // setCount(characters.info.count); не можем тк не используем пагинацию =/ 
        setCount(characters.results.length);
      } catch {
        onCharactersUpdate(null);
        setCount(0);
      }
    }
  }, [query, onCharactersUpdate]);
    ...
    <div className="w-full flex justify-center">
      <div className="w-full max-w-full lg:max-w-[600px] flex flex-col gap-1 sm:gap-2 justify-start">
        <input
          ...
          className="font-fira-bold text-sm sm:text-md text-primary-purple placeholder:text-primary-purple px-[15px] sm:px-[25px] py-[10px] sm:py-[20px] shadow-[0px_7px_12px] shadow-secondary-gray/20 w-full"
          type="text"
